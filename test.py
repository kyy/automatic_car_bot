...
class PDF(FPDF):
        ...
        for row in rows:
            ...
            for datum in row:
                ...
                self.multi_cell(
                    w=col_widths[i],
                    h=line_height,
                    txt=datum[:50],  # ограничиваем длину до 50 символов
                    align="L",
                    border=1,
                    new_x="RIGHT",
                    new_y="TOP",
                    max_line_height=self.font_size + 0.5,
                    link=link_to_car,
                    fill=fill,
                )
                i += 1
    ...
    def render_table_header(self, headings, col_widths):
        self.font_size_pt = 7  # размер шрифта в строка таблицы
...
async def do_pdf(
    ...
    if get_data():
        ...
        return pdf.output(f"logic/buffer/111.pdf")
...
def get_data():
    ...
    dataframe = pd.DataFrame(np.load(f"logic/buffer/111.npy", allow_pickle=True))
    if len(dataframe) > 0:
