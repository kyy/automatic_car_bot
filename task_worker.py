...
async def parse(ctx, car, message, name):
    av_link_json, abw_link_json, onliner_link_json = await all_get_url(car)
    parse_main(av_link_json, abw_link_json, onliner_link_json, message, name)
...
async def main():
    ...
    async with aiosqlite.connect(db_name) as db:
        select_cursor = await db.execute(f"""SELECT user.tel_id, udata.search_param, udata.id FROM udata
                                             INNER JOIN user on user.id = udata.user_id
                                             WHERE udata.is_active=1 """)
        select = await select_cursor.fetchall()
    for item in select:
        await redis.enqueue_job('parse', item[1][7:], item[0], item[2])
